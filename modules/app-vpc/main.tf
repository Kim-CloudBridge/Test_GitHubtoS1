...
# Application Subnets, Route Tables, Associations, Routes, ACLs and ACL Rules
#appvpc_application
resource "aws_subnet" "application_private_subnets" {
  count = length(var.environment.app_vpc.private_subnets_for_applications)

  vpc_id               = module.app_vpc.vpc_id
  cidr_block           = var.environment.app_vpc.private_subnets_for_applications[count.index]
  availability_zone    = length(regexall("^[a-z]{2}-", element(local.azs, count.index))) > 0 ? element(local.azs, count.index) : null
  availability_zone_id = length(regexall("^[a-z]{2}-", element(local.azs, count.index))) == 0 ? element(local.azs, count.index) : null

  tags = {
    Name = local.application_subnet_names[count.index]
  }
}
resource "aws_route_table" "application_route_table" {
  vpc_id = module.app_vpc.vpc_id

  tags = local.application_route_table_tags
}
resource "aws_route_table_association" "application_route_table_with_AZs" {
  count = length(var.environment.app_vpc.private_subnets_for_applications)

  subnet_id      = aws_subnet.application_private_subnets[count.index].id
  route_table_id = aws_route_table.application_route_table.id
}
# Routes for Transit Gateway for private route table
resource "aws_route" "app_route_0_0_0_0_0" {
  route_table_id         = aws_route_table.application_route_table.id
  destination_cidr_block = "0.0.0.0/0"
  transit_gateway_id     = var.transit_gateway_id
}
resource "aws_network_acl" "application_network_acl" {
  vpc_id     = module.app_vpc.vpc_id
  subnet_ids = aws_subnet.application_private_subnets[*].id

  tags = local.application_acl_tags
}
resource "aws_network_acl_rule" "application_inbound_acl_rules" {
  count = length(var.network_app_acls.application_inbound) > 0 ? length(var.network_app_acls.application_inbound) : 0

  network_acl_id  = aws_network_acl.application_network_acl.id
  egress          = false
  rule_number     = var.network_app_acls.application_inbound[count.index]["rule_number"]
  rule_action     = var.network_app_acls.application_inbound[count.index]["rule_action"]
  from_port       = lookup(var.network_app_acls.application_inbound[count.index], "from_port", null)
  to_port         = lookup(var.network_app_acls.application_inbound[count.index], "to_port", null)
  icmp_code       = lookup(var.network_app_acls.application_inbound[count.index], "icmp_code", null)
  icmp_type       = lookup(var.network_app_acls.application_inbound[count.index], "icmp_type", null)
  protocol        = var.network_app_acls.application_inbound[count.index]["protocol"]
  cidr_block      = lookup(var.network_app_acls.application_inbound[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.network_app_acls.application_inbound[count.index], "ipv6_cidr_block", null)
}
resource "aws_network_acl_rule" "application_outbound_acl_rules" {
  count = length(var.network_app_acls.application_outbound) > 0 ? length(var.network_app_acls.application_outbound) : 0

  network_acl_id  = aws_network_acl.application_network_acl.id
  egress          = true
  rule_number     = var.network_app_acls.application_outbound[count.index]["rule_number"]
  rule_action     = var.network_app_acls.application_outbound[count.index]["rule_action"]
  from_port       = lookup(var.network_app_acls.application_outbound[count.index], "from_port", null)
  to_port         = lookup(var.network_app_acls.application_outbound[count.index], "to_port", null)
  icmp_code       = lookup(var.network_app_acls.application_outbound[count.index], "icmp_code", null)
  icmp_type       = lookup(var.network_app_acls.application_outbound[count.index], "icmp_type", null)
  protocol        = var.network_app_acls.application_outbound[count.index]["protocol"]
  cidr_block      = lookup(var.network_app_acls.application_outbound[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.network_app_acls.application_outbound[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_route" "intra_route_0_0_0_0_0" {
  route_table_id         = module.app_vpc.intra_route_table_ids[0]
  destination_cidr_block = "0.0.0.0/0"
  transit_gateway_id     = var.transit_gateway_id
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw_attach" {
  subnet_ids         = [for subnet in aws_subnet.application_private_subnets : subnet.id]
  transit_gateway_id = var.transit_gateway_id
  vpc_id             = module.app_vpc.vpc_id
...
module "vpc_endpoint"{
  ...
  vpc_id = module.app_vpc.vpc_id
  route_table_ids = [
    aws_route_table.db_route_table.id,
    aws_route_table.application_route_table.id
  ]
