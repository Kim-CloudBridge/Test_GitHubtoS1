...
provider "aws" {
  ...
  # profile = "lendscape-shared-services"
  assume_role {
    role_arn     = "arn:aws:iam::${module.global_vars.global.aws_landing_zone.hms_management_utilities}:role/TerraformAdministratorServiceRoleCF"
...
provider "dme" {
  # dme Api key
  api_key    = local.dme_apikey
  # dme secret key
  secret_key = local.dme_secretkey
}